...
- [ ] **轨迹导出 JSON/CSV**：保存录制的轨迹数据
- [ ] **轨迹导入**：加载外部轨迹文件进行回放
- [ ] **截图功能**：一键保存当前 Canvas 截图
...
- [ ] 提取 `useMouseControl` hook（PLAN.md 中原计划，目前逻辑散落在 `ScleraClickHandler`）
- [ ] 轨迹数据结构改为 `{ alpha, beta, depth, tipPosition, timestamp }` 替代纯 `Vec3[]`
- [ ] 考虑将 `ScleraClickHandler` 拆分为 `RCMPlacer` + `NeedleDragger` + `DepthScroller` 三个独立处理器
...
| P2 | 3D 增强 | 后处理 (Bloom/DOF)、可视化增强 | 待实现 |
| P2 | 功能 | 触控、阶段完善、数据导出 | 待实现 |
